...
pub struct JwtRuntimeConfig {
    ...
    /// Prefix prepended to the JWT `kid` header to form the Redis key holding the public PEM.
        ...
        match self {
            Self::Redis { conn, key_prefix } => {
                ...
                match conn.get::<_, Option<String>>(&key).await {
                    ...
                    Err(err) => {
                        warn!("Redis GET failed for kid {}: {}", kid, err);
                        None
                    }
                }
            }
    ...
    pub async fn from_config(cfg: &JwtRuntimeConfig) -> anyhow::Result<Self> {
        ...
        Ok(Self {
            fetcher: KeyFetcher::Redis {
                conn,
                key_prefix: cfg.redis_key_prefix.clone(),
            },
