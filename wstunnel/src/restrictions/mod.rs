...
mod tests {
    ...
    fn runtime_consistency_passes_when_jwt_matcher_has_verifier() {
        ...
        rules.validate_runtime_consistency(true).expect("ok when verifier is present");
    }
    ...
    fn runtime_consistency_rejects_empty_allow_list_through_helper() {
        ...
        assert!(
            err.to_string().contains("empty allowed-values list"),
            "unexpected error: {err}"
        );
    }
...
"#;
        ...
        assert!(
            err.to_string().contains("required_claim"),
            "unexpected error: {err}"
        );
    }
}
