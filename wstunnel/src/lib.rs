...
async fn run_server_impl(args: Server, executor: impl TokioExecutorRef) -> anyhow::Result<()> {
    ...
    let jwt_verifier = if let Some(redis_url) = args.jwt_redis_url.as_deref() {
        let redis_keys_table = args
            .jwt_redis_keys_table
            .as_deref()
            .expect("--jwt-redis-keys-table is required when --jwt-redis-url is set");
        let cfg = crate::restrictions::jwt::JwtRuntimeConfig {
            redis_url: redis_url.to_string(),
            redis_keys_table: redis_keys_table.to_string(),
